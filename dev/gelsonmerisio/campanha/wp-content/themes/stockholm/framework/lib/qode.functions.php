...
if ( ! function_exists( 'qode_export_custom_sidebars' ) ) {
	function qode_export_custom_sidebars() {
...
if ( ! function_exists( 'qode_export_widgets_sidebars' ) ) {
	function qode_export_widgets_sidebars() {
		$data             = array();
		$data['sidebars'] = qode_export_sidebars();
		$data['widgets']  = qode_export_widgets();
...
if ( ! function_exists( 'qode_export_widgets' ) ) {
	function qode_export_widgets() {
		global $wp_registered_widgets;
...
if ( ! function_exists( 'qode_export_sidebars' ) ) {
	function qode_export_sidebars() {
		$sidebars = get_option( "sidebars_widgets" );
		$sidebars = qode_exclude_sidebar_keys( $sidebars );

		return $sidebars;
	}
}

if ( ! function_exists( 'qode_exclude_sidebar_keys' ) ) {
	function qode_exclude_sidebar_keys( $keys = array() ) {
		if ( ! is_array( $keys ) ) {
			return $keys;
		}
...
if ( ! function_exists( 'qode_import_custom_sidebars' ) ) {
		...
		if ( current_user_can( 'administrator' ) ) {
			...
			if ( empty( $_POST ) || ! isset( $_POST ) ) {
				...
				if ( wp_verify_nonce( $data['nonce'], 'qodef_import_custom_sidebars_secret_value' ) ) {
					...
					qode_ajax_status( 'success', esc_html__( 'Custom sidebars imported successfully', 'qode' ) );
	...
	add_action( 'wp_ajax_qode_import_custom_sidebars', 'qode_import_custom_sidebars' );
}

if ( ! function_exists( 'qode_import_widgets' ) ) {
	/**
	 * Function that import sidebars and widgets to db.
	 * It hooks to ajax wp_ajax_qode_import_sidebar_and_widgets action.
	 */
	function qode_import_widgets() {
		if ( current_user_can( 'administrator' ) ) {
			...
			if ( empty( $_POST ) || ! isset( $_POST ) ) {
				...
				if ( wp_verify_nonce( $data['nonce'], 'qodef_import_widgets_secret_value' ) ) {
					...
					qode_ajax_status( 'success', esc_html__( 'Widgets imported successfully', 'qode' ) );
				} else {
					qode_ajax_status( 'error', esc_html__( 'Non valid authorization', 'qode' ) );
				}
			}
	...
	add_action( 'wp_ajax_qode_import_widgets', 'qode_import_widgets' );
}
