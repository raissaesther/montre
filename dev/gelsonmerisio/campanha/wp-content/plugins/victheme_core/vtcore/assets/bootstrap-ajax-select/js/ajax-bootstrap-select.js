...
var AjaxBootstrapSelect = function (element, options) {
    ...
    // Add cancel button
    this.selectpicker.$cancelbutton = $('<button type="button" class="btn btn-cancel btn-danger">X</button>');
    this.selectpicker.$cancelbutton.insertAfter(this.selectpicker.$button);
    this.selectpicker.$newElement.addClass('bootstrap-ajax-picker');

    // Add empty option for resetting the form, without this PHP form will not post the select element
    // when reseted since it is invalid to post non-existance options
    this.$element.prepend('<option value="" class="hidden" data-hidden="true"></option>');
...
AjaxBootstrapSelect.prototype.init = function () {
    ...
    this.selectpicker.$cancelbutton.on('click', function() {
        plugin.selectpicker.val('');
    });
};
