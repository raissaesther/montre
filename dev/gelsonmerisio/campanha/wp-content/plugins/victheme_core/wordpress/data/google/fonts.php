...
extends VTCore_Wordpress_Models_Config {
  ...
  public function buildFontString() {
    ...
    if ($this->get('registered.fonts')) {
      foreach ($this->get('registered.fonts') as $delta => $font) {
        ...
        // Check against loaded fonts
        $loaded = $this->get('loaded.' . $font);

        // Building the correct variant
        foreach ($variant as $size) {

          // Don't load variants if it has been reported to be loaded previously
          if (!empty($loaded)
              && is_array($loaded)
              && in_array($size, $loaded)) {
            continue;
          }
        ...
        // Font loaded before and no new variant recorded skip this font
        if (!empty($loaded) && empty($match)) {
          continue;
        }

        // Registering current font available subsets
  ...
  public function loadFont($reset = false) {
    $this->remove('url')->buildFontString();
    return $this;
  }


  /**
   * Method for enqueueFont.
   * Only use this in Core!
   *
   * Core will invoke this in the wp_enqueue_scripts hook and wp_footer hook
   */
  public function enqueueFont() {
    if ($this->get('url')) {

      // Wordpress doesnt allow to reregister assets with the same name, use unique id!
      $id = uniqid('vtcore-google-fonts-');
      wp_enqueue_style($id, $this->get('url'), array(), NULL, 'all');

      $this->recordLoadedFonts();
      $this->cleanRegisteredFonts();
    }
  }


  /**
   * Flatten user registered weight and styles into a single arrays
   * @return array
   */
  ...
  /**
   * Record all loaded fonts so it wont get double enqueued
   * @return $this
   */
  private function recordLoadedFonts() {
    if ($this->get('registered.fonts')) {
      $variant = $this->buildVariant();
      foreach ($this->get('registered.fonts') as $delta => $font) {
        $this->add('loaded.' . $font, $variant);
      }
    }
    return $this;
  }
  ...
  /**
   * Clean the registered fonts into the default state
   * @return $this
   */
  private function cleanRegisteredFonts() {
    $this->mutate('registered', array(
      'styles' => array(),
      'weight' => array(),
      'fonts' => array(),
    ));
