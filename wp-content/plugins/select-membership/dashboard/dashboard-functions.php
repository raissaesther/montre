				...
				if ( $user_id ) {
					...
					if ( ! empty( $update_data['password'] ) ) {
						if ( $update_data['password'] === $update_data['password2'] ) {
							...
							qode_membership_ajax_response( 'error', esc_html__( 'Passwords don\'t match', 'qode_membership' ) );
						}
					}
					...
					if ( ! empty( $update_data['email'] ) && filter_var( $update_data['email'], FILTER_VALIDATE_EMAIL ) ) {
						...
						qode_membership_ajax_response( 'error', esc_html__( 'Error. Please insert valid email', 'qode_membership' ) );
					}
					...
					qode_membership_ajax_response( 'success', esc_html__( 'Your profile is updated', 'qode_membership' ), $dashboard_url );

				} else {
					qode_membership_ajax_response( 'error', esc_html__( 'You are unauthorized to perform this action.', 'qode_membership' ) );
				}

			} else {
				qode_membership_ajax_response( 'error', esc_html__( 'Error.', 'qode_membership' ) );
			}
		}
	}
