...
if ( ! function_exists( 'qode_membership_login_user' ) ) {
	...
	function qode_membership_login_user() {
		...
		if ( empty( $_POST ) || ! isset( $_POST ) ) {
			qode_membership_ajax_response( 'error', esc_html__( 'All fields are empty', 'qode_membership' ) );
		} else {
			...
			if ( is_wp_error( $user_signon ) ) {
				qode_membership_ajax_response( 'error', esc_html__( 'Wrong username or password.', 'qode_membership' ) );
			} else {
				if ( $redirect_uri == '' ) {
					$redirect_uri = qode_membership_get_my_account_page_url();
					if ( empty( $redirect_uri ) ) {
						$redirect_uri = esc_url( home_url( '/' ) );
					}
				}
				qode_membership_ajax_response( 'success', esc_html__( 'Login successful, redirecting...', 'qode_membership' ), $redirect_uri );
			}
		}
			...
			if ( ! $user_id && ! $user_email ) {
				if ( $credentials['password'] == $credentials['confirm_password'] ) {
					...
					if ( is_wp_error( $user_save_flag ) ) {
						qode_membership_ajax_response( 'error', esc_html__( 'Something went wrong', 'qode_membership' ) );
					} else {
						wp_update_user( array( 'ID' => $user_save_flag, 'role' => get_option( 'default_role' ) ) );
						$mail_to = $credentials['user_email'];
						
						$subject = esc_html__( 'User Registration', 'qode_membership' ); //Subject
						$message = esc_html__( 'You have registered successfully on ', 'qode_membership' ) . esc_url( get_site_url() ); //Message
						wp_mail( $mail_to, $subject, $message );
						qode_membership_ajax_response( 'success', esc_html__( 'You have registered successfully, please login with the created credentials', 'qode_membership' ) );
						
					}
				} else {
					qode_membership_ajax_response( 'error', esc_html__( 'Both passwords must match in order to register', 'qode_membership' ) );
				}
			} elseif ( $user_id ) {
				qode_membership_ajax_response( 'error', esc_html__( 'Username already exists', 'qode_membership' ) );
			} else {
				qode_membership_ajax_response( 'error', esc_html__( 'User with that name already exists', 'qode_membership' ) );
			}
		}
...
if ( ! function_exists( 'qode_membership_user_lost_password' ) ) {
	...
	function qode_membership_user_lost_password() {
		...
		if ( $result === true ) {
			qode_membership_ajax_response( 'success', esc_html__( 'We have sent you an email', 'qode_membership' ) );
		} else {
